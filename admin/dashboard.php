...
<body>
					...
					<ul class="nav nav-primary">
						...
						<li class="nav-item">
							...
							<div class="collapse" id="base">
								<ul class="nav nav-collapse">
									...
									<li>
										<a href="?page=walas">
											<span class="sub-item"> Kaprodi </span>
										</a>
									</li>
									<li>
										<a href="?page=wakadir">
											<span class="sub-item"> Wadir </span>
										</a>
									</li>
							...
							<div class="collapse" id="siswa">
								<ul class="nav nav-collapse">
									...
									foreach ($kelas as $k) { ?>
										...
										<li>
											<a href="?page=siswa&kelas=<?= $k['id_mkelas'] ?> ">
												<span class="sub-item">KELAS <?= strtoupper($k['nama_kelas']); ?> <?= strtoupper($k['semester']); ?></span>
											</a>
										</li>
									<?php } ?>
								</ul>
						...
						<li class="nav-item">
							...
							<div class="collapse" id="rekapAbsen">
								<ul class="nav nav-collapse">
									<?php
									$kelas = mysqli_query($con, "SELECT * FROM tb_prodi ORDER BY id_prodi ASC");
									foreach ($kelas as $k) { ?>
										<li>
											<a href="?page=rekap&kelas=<?= $k['id_prodi'] ?> ">
												<span class="sub-item">KELAS <?= strtoupper($k['prodi']); ?></span>
											</a>
										</li>
									<?php } ?>
								</ul>
			...
			<div class="content">
				...
				if ($page == 'master') {
					...
					if ($act == 'kelas') {
						...
						include 'modul/master/prodi/del.php';
						//ruangan
					} elseif ($act == 'ruangan') {
						include 'modul/master/ruangan/data_ruangan.php';
					} elseif ($act == 'delruangan') {
						include 'modul/master/ruangan/del.php';
						// semster
					} elseif ($act == 'semester') {
					...
					} elseif ($act == 'set_walas') {
						include 'modul/wakel/set.php';
					}
				} elseif ($page == 'wakadir') {
					if ($act == '') {
						include 'modul/wakadir/data.php';
					} elseif ($act == 'del') {
						include 'modul/wakadir/del.php';
					} elseif ($act == 'set_wadir') {
						include 'modul/wakadir/set.php';
					}
				} elseif ($page == 'kepsek') {
					if ($act == '') {
					...
					if ($act == '') {
						...
						include 'modul/siswa/process.php';
					}
				} elseif ($page == 'rekap') {
					if ($act == '') {
					...
					if ($act == '') {
						...
						include 'modul/jadwal/detail.php';
					} elseif ($act == 'cancel') {
	...
	<script>
		...
		startDateInput.max = nextYearDate;
	</script>
	<script>
		document.getElementById('selectAll').addEventListener('change', function() {
			const checkboxes = document.querySelectorAll('.siswa');
		...
		const students = document.querySelectorAll('.siswa');
		students.forEach(student => {
			student.addEventListener('change', function() {
				if (!this.checked) {
					document.getElementById('selectAll').checked = false;
				} else {
					const allChecked = Array.from(students).every(stu => stu.checked);
		...
		function toggleSubmitButton() {
			const anyChecked = Array.from(students).some(stu => stu.checked);
	...
	<!-- <script>
		function updateMataKuliahOptions() {
			...
			if (prodiId && semesterId) {
				...
				$('#mapel').html('<option value="">--Pilih Mata Kuliah--</option>');
			}
		} -->
	<!-- </script> -->
	<script>
		function updateMataKuliah() {
			...
			if (prodi !== "" && semester !== "") {
				// Menggunakan AJAX untuk mendapatkan data mata kuliah berdasarkan prodi dan semester
				var xhr = new XMLHttpRequest();
				xhr.open("GET", "../config/ajax_matkul.php?prodi=" + prodi + "&semester=" + semester, true);
				xhr.onload = function() {
					if (this.status === 200) {
						document.getElementById("mapel").innerHTML = this.responseText;
					}
				};
				xhr.send();
			}
	...
	<script>
		$(document).ready(function() {
			$('#mapel').change(function() {
				...
				if (mata_kuliah_id != "") {
					$.ajax({
						url: '../config/ajax_sks.php',
						type: 'post',
						data: {
							id_mapel: mata_kuliah_id
						},
						success: function(response) {
							$('#sks').val(response);
						}
	...
	<script>
		function updateMataKuliahOptions1() {
			...
			if (prodiId) {
				// Menggunakan AJAX untuk mengambil data kelas berdasarkan prodi
				var xhr = new XMLHttpRequest();
				xhr.open('GET', '../config/get_kelas.php?prodi_id=' + prodiId, true);
				xhr.onload = function() {
					if (xhr.status === 200) {
