...
WHERE tb_mengajar.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_prodi='$_GET[kelas]'  AND tb_thajaran.status=1 AND tb_semester.status=1
	...
	$walikelas = mysqli_query($con, "SELECT * FROM tb_walikelas INNER JOIN tb_guru ON tb_walikelas.id_guru=tb_guru.id_guru WHERE tb_walikelas.id_prodi='$_GET[kelas]' AND tb_walikelas.status='1' ");
 				...
 				<em style="font: 11px;">
					...
					for ($i = 1; $i <= $tglTerakhir; $i++) { 
						...
						$ket = mysqli_query($con, "SELECT * FROM _logabsensi
							...
							INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
							INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
							WHERE _logabsensi.pertemuan_ke='$i' 
						...
						foreach ($ket as $h) {
							$tanggalPertemuan = date('d-F-Y', strtotime($h['tgl_absen']));
						}
						echo "<td align='center' style='transform: rotate(0deg); transform-origin: center; font-size: 12px; height: 50px;  width: 50px;' ><em >$tanggalPertemuan</em></td>";
					}
