...
<table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
    ...
    ?>
        ...
        <tr bgcolor="<?= $warna; ?>">
			...
			INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
            INNER JOIN tb_prodi ON tb_mengajar.id_prodi=tb_prodi.id_prodi
			WHERE _logabsensi.pertemuan_ke='$i' AND _logabsensi.id_siswa='$ds[id_siswa]' AND _logabsensi.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_prodi='$_GET[prodi]'  AND tb_thajaran.status=1 AND tb_semester.status=1 GROUP BY pertemuan_ke ");
                    foreach ($ket as $h)
