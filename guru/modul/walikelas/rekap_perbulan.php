...
$kelasMengajar = mysqli_query($con, "SELECT * FROM tb_mengajar 
	...
	WHERE tb_mengajar.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_prodi='$_GET[prodi]'  AND tb_thajaran.status=1 AND tb_semester.status=1 ");

foreach ($kelasMengajar as $d)


    // tampilkan data walikelas
    $walikelas = mysqli_query($con, "SELECT * FROM tb_walikelas INNER JOIN tb_guru ON tb_walikelas.id_guru=tb_guru.id_guru WHERE tb_walikelas.id_prodi='$_GET[prodi]' ");
 			...
 			-->
                ...
                <div class="card-body">
                    <a target="_blank" href="../guru/modul/walikelas/rekap_persemester.php?pelajaran=<?= $_GET["pelajaran"] ?>&bulan=<?= $bulan; ?>&prodi=<?= $_GET["prodi"] ?>" class="btn btn-default">
                        ...
                        REKAP (<?= strtoupper($d['nama_kelas']); ?> <?= strtoupper($d['semester']); ?> - <?= strtoupper($d['tahun_ajaran']); ?>)
                    </a>
                    <?php
						...
						CETAK BULAN (<?php echo strtoupper(namaBulan($bulan)); ?>)
                            ...
                            <table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
                                <tr>
                                    ...
                                    <td colspan="<?= $tglTerakhir; ?>" style="padding: 8px;">PERTEMUAN KE- DAN BULAN : <b style="text-transform: uppercase;"><?php echo namaBulan($bulan); ?> <?= date('Y'); ?></b></td>
                                    ...
                                    <tr bgcolor="<?= $warna; ?>">
                                        ...
                                        ?>
                                            <td align="center" bgcolor="white">
                                                <?php
                                                $ket = mysqli_query($con, "SELECT * FROM _logabsensi WHERE  _logabsensi.pertemuan_ke='$i' AND id_siswa='$d[id_siswa]' AND id_mengajar='$_GET[pelajaran]' AND MONTH(tgl_absen)='$bulan' GROUP BY DAY(tgl_absen) ");
                                                foreach ($ket as $h)
