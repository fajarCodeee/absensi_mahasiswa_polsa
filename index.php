			...
			<div class="wrap-login100">
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if ($level == 1) {
						...
						//wakadir
						$sqlCek = mysqli_query($con, "SELECT * FROM wakadir 
						INNER JOIN tb_guru ON wakadir.id_guru=tb_guru.id_guru
						 WHERE nip='$_POST[username]' AND password='$pass' AND status='Y'");
						$jml = mysqli_num_rows($sqlCek);
						$d = mysqli_fetch_array($sqlCek);

						if ($jml > 0) {
							$_SESSION['wakadir'] = $d['id_wakadir'];
							...
							echo "
								...
								window.setTimeout(function(){ 
								window.location.replace('./wakadir/');
								} ,3000);   
								</script>";
						} else {
							echo "
